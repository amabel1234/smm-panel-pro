import { useState, useMemo } from "react";
  ...
  import { Loader2, ShoppingCart, ChevronDown, Info, CheckCircle2, Link as LinkIcon, Hash } from "lucide-react";
  import { SiInstagram, SiTiktok, SiYoutube, SiFacebook, SiTelegram, SiWhatsapp, SiX, SiSpotify } from "react-icons/si";

  function getPlatformIcon(name: string, size = "w-4 h-4") {
    const n = (name || "").toLowerCase();
    if (n.includes("instagram")) return <SiInstagram className={`${size} text-pink-400`} />;
    if (n.includes("tiktok"))    return <SiTiktok    className={`${size} text-white`} />;
    if (n.includes("youtube"))   return <SiYoutube   className={`${size} text-red-400`} />;
    if (n.includes("facebook"))  return <SiFacebook  className={`${size} text-blue-400`} />;
    if (n.includes("telegram"))  return <SiTelegram  className={`${size} text-sky-400`} />;
    if (n.includes("whatsapp"))  return <SiWhatsapp  className={`${size} text-green-400`} />;
    if (n.includes("twitter") || n.includes(" x ")) return <SiX className={`${size} text-white`} />;
    if (n.includes("spotify"))   return <SiSpotify   className={`${size} text-green-500`} />;
    return null;
  }

  function SelectField({
    label, required, value, onChange, placeholder, children, disabled
  }: {
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <select
            ...
            disabled={disabled}
            className={`w-full appearance-none px-4 py-3.5 rounded-xl bg-white/5 border text-sm transition-all focus:outline-none pr-10 ${
              value ? "border-primary/50 text-foreground" : "border-white/10 text-muted-foreground"
            } ${disabled ? "opacity-40 cursor-not-allowed" : "hover:border-white/20 focus:border-primary/70 cursor-pointer"}`}
          >
            <option value="" className="bg-[#0f0f1a]">{placeholder}</option>
            {children}
          </select>
          <ChevronDown className={`absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none ${disabled ? "text-muted-foreground/30" : "text-muted-foreground"}`} />
        </div>
      </div>
    );
  }
  ...
  export default function Services() {
    ...
    const handleOrder = () => {
      ...
      if (!qty || qty < (selectedService?.minOrder ?? 1)) {
        toast.error(`Minimum jumlah: ${(selectedService?.minOrder ?? 1).toLocaleString("id-ID")}`); return;
      }
      if (qty > (selectedService?.maxOrder ?? 999999)) {
        toast.error(`Maksimum jumlah: ${(selectedService?.maxOrder ?? 999999).toLocaleString("id-ID")}`); return;
      }
        ...
        <div className="space-y-4 max-w-xl mx-auto">
          ...
          <div className="glass rounded-2xl border border-white/10 p-5 space-y-5">
            ...
            <SelectField
              ...
              placeholder={isLoadingCategories ? "Memuat kategori..." : "Pilih Kategori"}
              disabled={isLoadingCategories}
            >
              {(categories as string[] | undefined)?.map((cat) => (
                <option key={cat} value={cat} className="bg-[#0f0f1a] flex items-center gap-2">
                  {cat}
                </option>
              ))}
            </SelectField>
            ...
            <SelectField
              ...
              placeholder={!selectedCategory ? "Pilih kategori dulu" : isLoadingServices ? "Memuat layanan..." : filteredServices.length === 0 ? "Tidak ada layanan" : "Pilih Layanan"}
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground flex items-center gap-1">
                <Info className="w-3 h-3 shrink-0" /> Pastikan akun/postingan bersifat <strong className="text-foreground">publik</strong>
              </p>
            </div>
            ...
            <div className="rounded-xl bg-white/5 border border-white/10 px-4 py-3.5 flex items-center justify-between">
              ...
              <span className={`text-xl font-bold ${totalPrice > 0 ? "text-primary" : "text-muted-foreground"`}>
            ...
            {totalPrice > 0 && (
              <div className={`rounded-xl px-4 py-2.5 flex items-center justify-between text-xs ${
                ...
                <span className={`font-semibold flex items-center gap-1 ${balance >= totalPrice ? "text-green-400" : "text-red-400"}`}>
                  {balance >= totalPrice && <CheckCircle2 className="w-3 h-3" />}
                  {formatRupiah(balance)}
                  {balance < totalPrice && <span className="text-xs">(kurang {formatRupiah(totalPrice - balance)})</span>}
                </span>
              </div>
            )}

import { AppLayout } from "@/components/layout/AppLayout";
  import { useListNokosCountries, useListNokosApps, useOrderNokosNumber, useListActiveNumbers } from "@workspace/api-client-react";
  import { useState } from "react";
  import { formatRupiah } from "@/lib/utils";
  import { toast } from "sonner";
  import { Phone, Globe, ChevronRight, Loader2, CheckCircle2, Clock, Copy, ArrowLeft, Smartphone, RefreshCw } from "lucide-react";
  ...
  export default function Nokos() {
    ...
    const [orderingAppId, setOrderingAppId] = useState<number | null>(null);
    ...
    const orderMutation = useOrderNokosNumber();

    const handleOrder = (appId: number, appName: string, price: number) => {
            ...
            <div className="glass rounded-2xl border border-green-400/20 overflow-hidden">
              <div className="p-4 border-b border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-sm">Nomor Aktif ({activeNumbers.length})</span>
                </div>
                  ...
                  <div key={n.id} className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{n.appName} · {n.country}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      {n.smsCode && (
                      ...
                      <button onClick={() => copyText(n.number)} className="p-1.5 rounded-lg hover:bg-white/10 transition-colors">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-semibold">Pilih Aplikasi <span className="text-muted-foreground font-normal">— {selectedCountryName}</span></h2>
                  <span className="text-xs text-muted-foreground">(Step 2/2)</span>
                </div>
              </div>

              {isLoadingApps ? <AppSkeleton /> : !apps || apps.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {apps.map((app: any) => (
                    <div key={app.id} className="glass rounded-xl border border-white/10 hover:border-primary/30 transition-all p-4 space-y-3">
                      ...
                      <button
                        onClick={() => handleOrder(app.id, app.name, app.price)}
                        disabled={orderMutation.isPending && orderingAppId === app.id}
                      ...
                      >
                        {orderMutation.isPending && orderingAppId === app.id
